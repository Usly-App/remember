...
function renderNode(
  ...
  abc: string, isSelected: boolean,
  imageUrl?: string | null, nodeId?: string
) {
  const sw = isSelected ? 2.5 : 1.8;

  // Image mode — use pattern fill
  if (imageUrl && nodeId) {
    ...
    <>
      ...
      {displayMode === 'abc' ? (
        <text
          ...
          fontSize={Math.max(10, innerSize * 2.2)} fontWeight={800}
...
function NodeAppearanceEditor({
  displayMode, setDisplayMode, outerShape, setOuterShape, outerColor, setOuterColor, outerSize, setOuterSize, outerSolid, setOuterSolid,
  innerShape, setInnerShape, innerColor, setInnerColor, innerSize, setInnerSize, innerSolid, setInnerSolid, abc, setAbc,
}: {
  displayMode: 'shape' | 'abc'; setDisplayMode: (v: 'shape' | 'abc') => void;
  outerShape: string; setOuterShape: (v: string) => void; outerColor: string; setOuterColor: (v: string) => void;
  outerSize: number; setOuterSize: (v: number) => void; outerSolid: boolean; setOuterSolid: (v: boolean) => void;
  innerShape: string; setInnerShape: (v: string) => void; innerColor: string; setInnerColor: (v: string) => void;
  innerSize: number; setInnerSize: (v: number) => void; innerSolid: boolean; setInnerSolid: (v: boolean) => void;
  ...
  return (
    <div className="space-y-4">
      <div>
        <label className={sL}>Display</label>
      ...
      <div className="bg-surface-container rounded-xl p-3 space-y-2">
        ...
        <div className="flex gap-4 items-center">
          ...
          <div><label className={subL}>Fill</label>
      ...
      <div className="bg-surface-container rounded-xl p-3 space-y-2">
        {displayMode === 'shape' ? (
          <>
            <label className={sL}>Inner Shape</label>
            <ShapePicker value={innerShape} onChange={setInnerShape} color={innerColor} />
            <div><label className={subL}>Colour</label><ColorPicker value={innerColor} onChange={setInnerColor} /></div>
            <div className="flex gap-4 items-center">
              <div className="flex-1"><label className={subL}>Size ({innerSize}px)</label><input type="range" min={2} max={Math.floor(outerSize * 0.7)} value={innerSize} onChange={(e) => setInnerSize(Number(e.target.value))} className="w-full" style={{ accentColor: innerColor }} /></div>
              <div><label className={subL}>Fill</label>
                <div className="flex gap-1">
          ...
          <>
            <label className={sL}>Characters (max 2)</label>
            ...
            <div><label className={subL}>Text Colour</label><ColorPicker value={innerColor} onChange={setInnerColor} /></div>
            <div><label className={subL}>Text Size ({innerSize}px)</label><input type="range" min={8} max={Math.floor(outerSize * 0.8)} value={innerSize} onChange={(e) => setInnerSize(Number(e.target.value))} className="w-full" style={{ accentColor: innerColor }} /></div>
          </>
        )}
      </div>

      <div>
        <label className={sL}>Preview</label>
...
function useNodeForm(initial: NodeFormState) {
  ...
  const selectPreset = (p: typeof NODE_PRESETS[0]) => {
    setForm((prev) => ({
      ...
      outerShape: p.outer_shape, outerColor: p.outer_color, outerSize: p.outer_size, outerSolid: p.outer_solid,
      innerShape: p.inner_shape, innerColor: p.inner_color, innerSize: p.inner_size, innerSolid: p.inner_solid,
    }));
    setCustomMode(false);
  };
      ...
      {customMode && (
        <>
          ...
          <NodeAppearanceEditor
            displayMode={form.displayMode} setDisplayMode={(v) => setForm((prev) => ({ ...prev, displayMode: v }))}
            outerShape={form.outerShape} setOuterShape={(v) => setForm((prev) => ({ ...prev, outerShape: v }))}
            outerColor={form.outerColor} setOuterColor={(v) => setForm((prev) => ({ ...prev, outerColor: v }))}
            outerSize={form.outerSize} setOuterSize={(v) => setForm((prev) => ({ ...prev, outerSize: v }))}
            outerSolid={form.outerSolid} setOuterSolid={(v) => setForm((prev) => ({ ...prev, outerSolid: v }))}
            innerShape={form.innerShape} setInnerShape={(v) => setForm((prev) => ({ ...prev, innerShape: v }))}
            innerColor={form.innerColor} setInnerColor={(v) => setForm((prev) => ({ ...prev, innerColor: v }))}
            innerSize={form.innerSize} setInnerSize={(v) => setForm((prev) => ({ ...prev, innerSize: v }))}
            innerSolid={form.innerSolid} setInnerSolid={(v) => setForm((prev) => ({ ...prev, innerSolid: v }))}
            abc={form.abc} setAbc={(v) => setForm((prev) => ({ ...prev, abc: v }))}
          />
        </>
      )}
    </>
...
function AddNodeModal({ parentNode, settings, onAdd, onClose }: { parentNode: MapNode; settings: UserSettings | null; onAdd: (data: any) => void; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    if (!nf.form.abc || nf.form.abc.length <= 1) {
      nf.setForm((prev) => ({ ...prev, abc: initial }));
    }
  }, [name]);
          ...
          <div>
            ...
            <div className="max-h-[120px] overflow-y-auto custom-scrollbar border border-outline-variant rounded-xl">
              {filteredNodes.map((n) => {
                ...
                return (
                  <button key={n.id} onClick={() => { setParentId(n.id); setSearch(''); }} className={`w-full flex items-center gap-2.5 px-3 py-2 text-left text-sm transition-colors ${parentId === n.id ? 'bg-primary/10 text-primary' : 'hover:bg-surface-container text-on-surface'}`}>
                    <ShapePreview shape={getNodeOuterShape(n)} color={nc} size={14} />
                    <span className="font-headline font-medium truncate">{n.name}</span>
                    {parentId === n.id && <span className="ml-auto text-xs">✓</span>}
                  </button>
                );
              })}
            </div>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-5">
        {!editing ? (
          <>
            ...
            <div className="mb-4">
              <label className="block text-xs font-headline font-semibold text-on-surface-variant uppercase tracking-wider mb-2">Image</label>
              <ImageUploadButton userId={userId} nodeId={node.id} currentUrl={node.image_url} onUploaded={(url) => onUpdate(node.id, { image_url: url })} />
            </div>
            {children.length > 0 && (
              <div className="mt-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {children.map((c) => {
                    const cc = getNodeOuterColor(c, settings);
                    return <span key={c.id} className="inline-flex items-center gap-1.5 text-xs font-headline font-medium px-3 py-1.5 rounded-full border" style={{ borderColor: cc + '40', color: cc }}><ShapePreview shape={getNodeOuterShape(c)} color={cc} size={12} /> {c.name}</span>;
                  })}
                </div>
              </div>
            )}
...
function MapCanvas({ nodes, positions, selectedId, highlightId, settings, dragEnabled, collapsedIds, childCountMap, onSelectNode, onDragNode, onToggleCollapse }: {
  ...
  const onDown = (e: React.MouseEvent | React.TouchEvent) => {
    ...
    if (nel && dragEnabled) {
      ...
      if (np) { setDraggingNodeId(nid); dragStart.current = p; nodeStartPos.current = { x: np.x, y: np.y }; didDrag.current = false; e.preventDefault(); return; }
    }
          ...
          {visibleNodes.map((n) => {
            ...
            return (
              <g key={n.id} data-node-id={n.id} style={{ cursor: dragEnabled ? (isDrag ? 'grabbing' : 'grab') : 'pointer', opacity: dimmed ? 0.2 : 1, transition: 'opacity 0.3s' }} onClick={(e) => onClick(e, n.id)}>
                ...
                <text x={p.x} y={p.y + oz + 16} textAnchor="middle" fill="#1c1b1b" fontSize={ir ? 13 : 11} fontWeight={ir ? 700 : 500} fontFamily="Manrope, system-ui, sans-serif">
                  {n.name.length > 18 ? n.name.slice(0, 17) + '…' : n.name}
                </text>
                {n.hint && !ir && (
                  <text x={p.x} y={p.y + oz + 29} textAnchor="middle" fill="#777587" fontSize={9} fontFamily="Inter, system-ui, sans-serif">
...
export default function MapCanvasPage() {
  ...
  return (
    <div className="relative flex-1 h-full">
      ...
      {/* Top controls — row 2 */}
      <div className="absolute top-[60px] left-4 z-10 flex gap-2 flex-wrap">
        <button onClick={() => setDragEnabled(!dragEnabled)} className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-headline font-semibold border shadow-sm transition-all ${dragEnabled ? 'bg-primary text-white border-primary shadow-md' : 'bg-surface-container-lowest border-surface-container-high text-on-surface-variant hover:text-primary'}`}>
          {dragEnabled ? <Unlock size={15} /> : <Lock size={15} />}{dragEnabled ? 'Moving' : 'Move'}
        </button>
        <button onClick={() => setQuickAddOpen(true)} className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-headline font-semibold border shadow-sm bg-surface-container-lowest border-surface-container-high text-on-surface-variant hover:text-primary transition-all">
          <Plus size={15} /> Add
        </button>
