...
function shapeClipPath(cx: number, cy: number, r: number, shape: string, clipId: string) {
  switch (shape) {
    case 'square': return <clipPath id={clipId}><rect x={cx - r} y={cy - r} width={r * 2} height={r * 2} rx={r * 0.22} /></clipPath>;
    case 'diamond': return <clipPath id={clipId}><polygon points={`${cx},${cy - r} ${cx + r},${cy} ${cx},${cy + r} ${cx - r},${cy}`} /></clipPath>;
    case 'triangle': return <clipPath id={clipId}><polygon points={`${cx},${cy - r} ${cx + r * 0.866},${cy + r * 0.5} ${cx - r * 0.866},${cy + r * 0.5}`} /></clipPath>;
    case 'hexagon': { const pts = Array.from({ length: 6 }, (_, i) => { const a = (Math.PI / 3) * i - Math.PI / 2; return `${cx + r * Math.cos(a)},${cy + r * Math.sin(a)}`; }).join(' '); return <clipPath id={clipId}><polygon points={pts} /></clipPath>; }
    case 'star': { const pts = Array.from({ length: 10 }, (_, i) => { const a = (Math.PI / 5) * i - Math.PI / 2; const rd = i % 2 === 0 ? r : r * 0.45; return `${cx + rd * Math.cos(a)},${cy + rd * Math.sin(a)}`; }).join(' '); return <clipPath id={clipId}><polygon points={pts} /></clipPath>; }
    case 'pentagon': { const pts = Array.from({ length: 5 }, (_, i) => { const a = (Math.PI * 2 / 5) * i - Math.PI / 2; return `${cx + r * Math.cos(a)},${cy + r * Math.sin(a)}`; }).join(' '); return <clipPath id={clipId}><polygon points={pts} /></clipPath>; }
    case 'octagon': { const pts = Array.from({ length: 8 }, (_, i) => { const a = (Math.PI / 4) * i - Math.PI / 8; return `${cx + r * Math.cos(a)},${cy + r * Math.sin(a)}`; }).join(' '); return <clipPath id={clipId}><polygon points={pts} /></clipPath>; }
    default: return <clipPath id={clipId}><circle cx={cx} cy={cy} r={r} /></clipPath>;
  }
}

function renderNode(cx: number, cy: number, os: string, oc: string, oz: number, oSolid: boolean, dm: 'shape' | 'abc', is2: string, ic: string, iz: number, iSolid: boolean, ab: string, isSel: boolean, imageUrl?: string | null, nodeId?: string) {
  if (imageUrl) {
    const clipId = `img-clip-${nodeId || Math.random()}`;
    return (<><defs>{shapeClipPath(cx, cy, oz, os, clipId)}</defs>{shapeElement(cx, cy, oz, os, oc, oc, isSel ? 2.5 : 1.8, false)}<image href={imageUrl} x={cx - oz} y={cy - oz} width={oz * 2} height={oz * 2} clipPath={`url(#${clipId})`} preserveAspectRatio="xMidYMid slice" />{shapeElement(cx, cy, oz, os, 'none', oc, isSel ? 2.5 : 1.8, false)}</>);
  }
  return (<>{shapeElement(cx, cy, oz, os, oc, oc, isSel ? 2.5 : 1.8, oSolid)}{dm === 'abc' ? <text x={cx} y={cy + 1} textAnchor="middle" dominantBaseline="central" fill={oSolid ? 'white' : ic} fontSize={Math.max(10, iz * 2.2)} fontWeight={800} fontFamily="Manrope, system-ui, sans-serif">{ab.slice(0, 2)}</text> : shapeElement(cx, cy, iz, is2, ic, ic, 0, iSolid)}</>);
}
...
function ColorPicker({ value, onChange }: { value: string; onChange: (c: string) => void }) {
  return (<div className="flex flex-wrap gap-1.5">{NODE_COLORS.map((c) => (<button key={c} onClick={() => onChange(c)} className="w-7 h-7 rounded-lg border-2 transition-all flex items-center justify-center" style={{ background: c, borderColor: value === c ? '#1c1b1b' : 'transparent', transform: value === c ? 'scale(1.15)' : 'scale(1)' }}>{value === c && <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12" /></svg>}</button>))}<label className="w-7 h-7 rounded-lg border-2 border-outline-variant cursor-pointer overflow-hidden relative"><input type="color" value={value} onChange={(e) => onChange(e.target.value)} className="absolute inset-0 opacity-0 cursor-pointer" /><div className="w-full h-full flex items-center justify-center"><Palette size={12} className="text-on-surface-variant" /></div></label></div>);
}

function ShapePicker({ value, onChange, color }: { value: string; onChange: (s: string) => void; color: string }) {
  return (<div className="grid grid-cols-4 gap-1.5">{SHAPES.map((s) => (<button key={s.key} onClick={() => onChange(s.key)} className="flex flex-col items-center gap-0.5 py-2 px-1 rounded-xl border-2 transition-all" style={{ borderColor: value === s.key ? color : 'transparent', background: value === s.key ? color + '10' : '#f0edec' }}><ShapePreview shape={s.key} color={value === s.key ? color : '#999'} size={20} /><span className="text-[9px] font-headline" style={{ color: value === s.key ? color : '#999' }}>{s.label}</span></button>))}</div>);
}
...
// ─── Onboarding (center node setup for a new map) ───────────────────
function MapOnboarding({ mapName, onComplete }: { mapName: string; onComplete: (name: string) => void }) {
  const [name, setName] = useState('');
  return (
    <div className="flex-1 flex items-center justify-center px-6"><div className="w-full max-w-sm page-enter text-center">
...
function useNodeForm(initial: NodeFormState) {
  const [customMode, setCustomMode] = useState(false); const [form, setForm] = useState(initial);
...
function AddNodeModal({ parentNode, settings, onAdd, onClose }: { parentNode: MapNode; settings: UserSettings | null; onAdd: (data: any) => void; onClose: () => void }) {
  const dp = NODE_PRESETS[0]; const nf = useNodeForm({ type: dp.type, displayMode: dp.display_mode, outerShape: dp.outer_shape, outerColor: dp.outer_color, outerSize: dp.outer_size, outerSolid: dp.outer_solid, innerShape: dp.inner_shape, innerColor: dp.inner_color, innerSize: dp.inner_size, innerSolid: dp.inner_solid, abc: '' });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm" onClick={onClose}><div className="bg-surface-container-lowest rounded-2xl shadow-2xl border border-surface-container-high w-full max-w-md mx-4 max-h-[85vh] overflow-y-auto custom-scrollbar animate-slide-up" onClick={(e) => e.stopPropagation()}>
...
function QuickAddModal({ nodes, settings, onAdd, onClose }: { nodes: MapNode[]; settings: UserSettings | null; onAdd: (data: any) => void; onClose: () => void }) {
  const dp = NODE_PRESETS[0]; const nf = useNodeForm({ type: dp.type, displayMode: dp.display_mode, outerShape: dp.outer_shape, outerColor: dp.outer_color, outerSize: dp.outer_size, outerSolid: dp.outer_solid, innerShape: dp.inner_shape, innerColor: dp.inner_color, innerSize: dp.inner_size, innerSolid: dp.inner_solid, abc: '' });
  const [parentId, setParentId] = useState(nodes.find((n) => !n.parent_id)?.id || ''); const [name, setName] = useState(''); const [hint, setHint] = useState(''); const [search, setSearch] = useState('');
  const filteredNodes = search.trim() ? nodes.filter((n) => n.name.toLowerCase().includes(search.toLowerCase())) : nodes; const selectedParent = nodes.find((n) => n.id === parentId);
  useEffect(() => { if (!nf.form.abc || nf.form.abc.length <= 1) nf.setForm({ ...nf.form, abc: name.charAt(0).toUpperCase() || '' }); }, [name]);
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm" onClick={onClose}><div className="bg-surface-container-lowest rounded-2xl shadow-2xl border border-surface-container-high w-full max-w-md mx-4 max-h-[85vh] overflow-y-auto custom-scrollbar animate-slide-up" onClick={(e) => e.stopPropagation()}>
      ...
      <div className="p-5 space-y-4">
        <div><label className="block text-xs font-headline font-semibold text-on-surface-variant uppercase tracking-wider mb-2">Add to</label><input value={search} onChange={(e) => setSearch(e.target.value)} className="w-full px-4 py-2.5 rounded-xl border border-outline-variant bg-white text-on-surface font-body text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 transition mb-2" placeholder="Search nodes…" /><div className="max-h-[120px] overflow-y-auto custom-scrollbar border border-outline-variant rounded-xl">{filteredNodes.map((n) => { const nc = getNodeOuterColor(n, settings); return <button key={n.id} onClick={() => { setParentId(n.id); setSearch(''); }} className={`w-full flex items-center gap-2.5 px-3 py-2 text-left text-sm transition-colors ${parentId === n.id ? 'bg-primary/10 text-primary' : 'hover:bg-surface-container text-on-surface'}`}><ShapePreview shape={getNodeOuterShape(n)} color={nc} size={14} /><span className="font-headline font-medium truncate">{n.name}</span>{parentId === n.id && <span className="ml-auto text-xs">✓</span>}</button>; })}</div>{selectedParent && <p className="text-xs text-on-surface-variant mt-1.5">Adding to <span style={{ color: getNodeOuterColor(selectedParent, settings) }} className="font-semibold">{selectedParent.name}</span></p>}</div>
        <TypePickerSection {...nf} />
...
function NodePanel({ node, nodes, settings, userId, onUpdate, onDelete, onAddChild, onResetPosition, onToggleCollapse, onClose }: { node: MapNode; nodes: MapNode[]; settings: UserSettings | null; userId: string; onUpdate: (id: string, data: Partial<MapNode>) => void; onDelete: (id: string) => void; onAddChild: (parentId: string) => void; onResetPosition: (id: string) => void; onToggleCollapse: (id: string) => void; onClose: () => void }) {
  const [editing, setEditing] = useState(false); const [form, setForm] = useState({ name: node.name, hint: node.hint || '', description: node.description || '', address: node.address || '', relationship: node.relationship || '' });
  ...
  const nodeColor = getNodeOuterColor(node, settings); const children = nodes.filter((n) => n.parent_id === node.id); const parent = nodes.find((n) => n.id === node.parent_id); const hasCustomPosition = node.position_x != null && node.position_y != null;

  useEffect(() => { setForm({ name: node.name, hint: node.hint || '', description: node.description || '', address: node.address || '', relationship: node.relationship || '' }); setEditing(false); nf.setForm({ type: node.type, displayMode: getNodeDisplayMode(node), outerShape: getNodeOuterShape(node), outerColor: getNodeOuterColor(node, settings), outerSize: getNodeOuterSize(node, isRoot), outerSolid: getNodeOuterSolid(node, isRoot), innerShape: getNodeInnerShape(node), innerColor: getNodeInnerColor(node, settings), innerSize: getNodeInnerSize(node, isRoot), innerSolid: getNodeInnerSolid(node), abc: getNodeAbc(node) }); nf.setCustomMode(false); }, [node, settings]);
  ...
  return (
    <div className="absolute top-0 right-0 w-[380px] max-w-[90vw] h-full bg-surface-container-lowest border-l border-surface-container-high z-40 flex flex-col animate-slide-right shadow-2xl">
      <div className="flex items-center justify-between p-5 border-b border-surface-container"><div className="flex items-center gap-3"><span className="inline-flex items-center gap-1.5 text-xs font-headline font-bold uppercase tracking-wider px-3 py-1 rounded-full" style={{ background: nodeColor + '18', color: nodeColor }}><ShapePreview shape={getNodeOuterShape(node)} color={nodeColor} size={14} /> {node.type}</span>{parent && <span className="text-xs text-on-surface-variant flex items-center gap-1"><ChevronRight size={12} /> {parent.name}</span>}</div><button onClick={onClose} className="text-on-surface/40 hover:text-on-surface transition-colors"><X size={20} /></button></div>
...
function SearchBar({ nodes, settings, onSelect }: { nodes: MapNode[]; settings: UserSettings | null; onSelect: (id: string) => void }) {
  const [open, setOpen] = useState(false); const [query, setQuery] = useState(''); const inputRef = useRef<HTMLInputElement>(null);
  const results = useMemo(() => { if (!query.trim()) return []; const q = query.toLowerCase(); return nodes.filter((n) => n.name.toLowerCase().includes(q) || (n.hint && n.hint.toLowerCase().includes(q)) || (n.description && n.description.toLowerCase().includes(q)) || n.type.toLowerCase().includes(q)).slice(0, 8); }, [query, nodes]);
  useEffect(() => { if (open && inputRef.current) inputRef.current.focus(); }, [open]);
  useEffect(() => { const h = (e: KeyboardEvent) => { if ((e.metaKey || e.ctrlKey) && e.key === 'k') { e.preventDefault(); setOpen(true); } if (e.key === 'Escape') { setOpen(false); setQuery(''); } }; window.addEventListener('keydown', h); return () => window.removeEventListener('keydown', h); }, []);
  if (!open) return <button onClick={() => setOpen(true)} className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-headline font-semibold border shadow-sm bg-surface-container-lowest border-surface-container-high text-on-surface-variant hover:text-primary transition-all"><Search size={15} /><span className="hidden sm:inline">Search</span><kbd className="hidden sm:inline text-[10px] bg-surface-container px-1.5 py-0.5 rounded font-mono text-on-surface-variant/60">⌘K</kbd></button>;
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-[15vh] bg-black/20 backdrop-blur-sm" onClick={() => { setOpen(false); setQuery(''); }}><div className="bg-surface-container-lowest rounded-2xl shadow-2xl border border-surface-container-high w-full max-w-lg mx-4 animate-slide-up" onClick={(e) => e.stopPropagation()}>
      <div className="flex items-center gap-3 px-5 py-4 border-b border-surface-container"><Search size={18} className="text-on-surface-variant flex-shrink-0" /><input ref={inputRef} value={query} onChange={(e) => setQuery(e.target.value)} className="flex-1 bg-transparent text-on-surface font-body text-base focus:outline-none" placeholder="Search nodes by name, hint, type…" />{query && <button onClick={() => setQuery('')} className="text-on-surface-variant/40 hover:text-on-surface transition"><X size={16} /></button>}<kbd className="text-[10px] bg-surface-container px-1.5 py-0.5 rounded font-mono text-on-surface-variant/50">ESC</kbd></div>
      {query.trim() ? (<div className="max-h-[300px] overflow-y-auto custom-scrollbar">{results.length > 0 ? results.map((n) => { const nc = getNodeOuterColor(n, settings); return (<button key={n.id} onClick={() => { onSelect(n.id); setOpen(false); setQuery(''); }} className="w-full flex items-center gap-3 px-5 py-3 text-left hover:bg-surface-container transition-colors"><ShapePreview shape={getNodeOuterShape(n)} color={nc} size={20} solid={getNodeOuterSolid(n, !n.parent_id)} /><div className="flex-1 min-w-0"><p className="font-headline font-semibold text-sm text-on-surface truncate">{n.name}</p><p className="text-xs text-on-surface-variant truncate">{n.type.charAt(0).toUpperCase() + n.type.slice(1)}{n.hint && ` · ${n.hint}`}</p></div><span style={{ color: nc }} className="text-xs font-headline">→</span></button>); }) : <div className="px-5 py-8 text-center text-on-surface-variant text-sm">No nodes found for &quot;{query}&quot;</div>}</div>) : <div className="px-5 py-6 text-center text-on-surface-variant/60 text-sm">Start typing to search across all nodes</div>}
    </div></div>
  );
}
...
function MapCanvas({ nodes, positions, selectedId, highlightId, settings, dragEnabled, collapsedIds, childCountMap, onSelectNode, onDragNode, onToggleCollapse }: { nodes: MapNode[]; positions: Record<string, { x: number; y: number }>; selectedId: string | null; highlightId: string | null; settings: UserSettings | null; dragEnabled: boolean; collapsedIds: Set<string>; childCountMap: Record<string, number>; onSelectNode: (id: string | null) => void; onDragNode: (id: string, x: number, y: number) => void; onToggleCollapse: (id: string) => void }) {
  const svgRef = useRef<SVGSVGElement>(null); const [pan, setPan] = useState({ x: 0, y: 0 }); const [zoom, setZoom] = useState(1);
  const [draggingCanvas, setDraggingCanvas] = useState(false); const [draggingNodeId, setDraggingNodeId] = useState<string | null>(null);
  ...
  const hiddenIds = useMemo(() => { const h = new Set<string>(); const hide = (pid: string) => { nodes.filter((n) => n.parent_id === pid).forEach((n) => { h.add(n.id); hide(n.id); }); }; collapsedIds.forEach((id) => hide(id)); return h; }, [collapsedIds, nodes]);
    ...
    <div className="relative w-full h-full">
      <svg ref={svgRef} className="w-full h-full" style={{ cursor: draggingNodeId || draggingCanvas ? 'grabbing' : 'grab' }} onMouseDown={onDown} onMouseMove={onMove} onMouseUp={onUp} onMouseLeave={onUp} onTouchStart={onDown} onTouchMove={onMove} onTouchEnd={onUp}>
        ...
        <g transform={`translate(${pan.x}, ${pan.y}) scale(${zoom})`}>
          {visibleNodes.filter((n) => n.parent_id && positions[n.id] && positions[n.parent_id]).map((n) => { const f = positions[n.parent_id!]; const t = positions[n.id]; const c = getNodeOuterColor(n, settings); const dim = isHL && n.id !== highlightId && n.parent_id !== highlightId; return <line key={`e-${n.id}`} x1={f.x} y1={f.y} x2={t.x} y2={t.y} stroke={c} strokeWidth={1.5} opacity={dim ? 0.08 : 0.25} />; })}
          {visibleNodes.map((n) => {
            const p = positions[n.id]; if (!p) return null;
            const ir = !n.parent_id; const oc = getNodeOuterColor(n, settings); const ic = getNodeInnerColor(n, settings);
            const os = getNodeOuterShape(n); const is2 = getNodeInnerShape(n); const oz = getNodeOuterSize(n, ir); const iz = getNodeInnerSize(n, ir);
            const oSolid = getNodeOuterSolid(n, ir); const iSolid = getNodeInnerSolid(n); const dm = getNodeDisplayMode(n); const ab = getNodeAbc(n);
            const isSel = selectedId === n.id; const isDrag = draggingNodeId === n.id; const isHLNode = highlightId === n.id; const dimmed = isHL && !isHLNode;
            ...
            return (
              <g key={n.id} data-node-id={n.id} style={{ cursor: dragEnabled ? (isDrag ? 'grabbing' : 'grab') : 'pointer', opacity: dimmed ? 0.2 : 1, transition: 'opacity 0.3s' }} onClick={(e) => onClick(e, n.id)}>
                ...
                {count > 0 && (<g onClick={(e) => { e.stopPropagation(); onToggleCollapse(n.id); }} style={{ cursor: 'pointer' }}><circle cx={p.x + oz * 0.7} cy={p.y - oz * 0.7} r={9} fill={isCollapsed ? oc : '#fcf9f8'} stroke={oc} strokeWidth={1.5} /><text x={p.x + oz * 0.7} y={p.y - oz * 0.7 + 1} textAnchor="middle" dominantBaseline="central" fill={isCollapsed ? 'white' : oc} fontSize={9} fontWeight={700} fontFamily="Manrope, system-ui, sans-serif">{count}</text></g>)}
              </g>
            );
          })}
        </g>
      ...
      <div className="absolute bottom-5 right-5 flex gap-3 flex-wrap bg-surface-container-lowest/90 backdrop-blur-sm border border-surface-container-high rounded-full px-4 py-2 z-10">{uniqueTypes.map((t) => { const s = visibleNodes.find((n) => n.type === t); if (!s) return null; const c = getNodeOuterColor(s, settings); return <span key={t} className="flex items-center gap-1.5 text-[11px] font-headline text-on-surface-variant"><ShapePreview shape={getNodeOuterShape(s)} color={c} size={12} />{t.charAt(0).toUpperCase() + t.slice(1)}</span>; })}</div>
...
export default function MapCanvasPage() {
  ...
  const childCountMap = useMemo(() => { const m: Record<string, number> = {}; nodes.forEach((n) => { if (n.parent_id) m[n.parent_id] = (m[n.parent_id] || 0) + 1; }); return m; }, [nodes]);
  ...
  const handleToggleCollapse = useCallback((id: string) => { setCollapsedIds((prev) => { const next = new Set(prev); if (next.has(id)) next.delete(id); else next.add(id); return next; }); }, []);
  const handleSearchSelect = useCallback((id: string) => { const toUncollapse = new Set<string>(); let cur = nodes.find((n) => n.id === id); while (cur?.parent_id) { if (collapsedIds.has(cur.parent_id)) toUncollapse.add(cur.parent_id); cur = nodes.find((n) => n.id === cur!.parent_id); } if (toUncollapse.size > 0) setCollapsedIds((prev) => { const next = new Set(prev); toUncollapse.forEach((id) => next.delete(id)); return next; }); setSelectedId(id); setHighlightId(id); setTimeout(() => setHighlightId(null), 2500); }, [nodes, collapsedIds]);
  ...
  if (!hasRootNode) return <MapOnboarding mapName={currentMap.name} onComplete={async (name) => { await addNode({ parent_id: null, map_id: mapId, name, type: 'user', hint: null, description: null, address: null, relationship: null, meta: {}, position_x: null, position_y: null, color: settings?.accent_color || '#3525cd', display_mode: 'abc', shape: 'circle', abc: name.charAt(0).toUpperCase(), outer_shape: 'circle', outer_color: settings?.accent_color || '#3525cd', outer_size: 38, outer_solid: true, inner_shape: 'circle', inner_color: '#ffffff', inner_size: 16, inner_solid: true, image_url: null }); }} />;

  const buildNodeData = (data: any) => ({ parent_id: data.parentId || null, map_id: mapId, name: data.name, type: data.type, hint: data.hint || null, description: data.description || null, address: data.address || null, relationship: data.relationship || null, meta: {}, position_x: null, position_y: null, color: data.outerColor, display_mode: data.displayMode, shape: data.outerShape, abc: data.abc || data.name.charAt(0).toUpperCase(), outer_shape: data.outerShape, outer_color: data.outerColor, outer_size: data.outerSize, outer_solid: data.outerSolid, inner_shape: data.innerShape, inner_color: data.innerColor, inner_size: data.innerSize, inner_solid: data.innerSolid, image_url: null });
    ...
    <div className="relative flex-1 h-full">
      ...
      <div className="absolute top-4 left-4 z-10 flex gap-2 flex-wrap">
        <button onClick={() => router.push('/map')} className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-sm font-headline font-semibold border shadow-sm bg-surface-container-lowest border-surface-container-high text-on-surface-variant hover:text-primary transition-all">
          <ChevronLeft size={15} /> <span className="hidden sm:inline">My Maps</span>
        </button>
      ...
      <div className="absolute top-14 sm:top-4 sm:left-auto sm:right-auto left-4 z-10 flex gap-2 flex-wrap" style={{ top: undefined }}>
        {/* Second row of controls — placed below the back button row on mobile */}
      </div>
      <div className="absolute top-[60px] sm:top-4 left-4 sm:left-[340px] z-10 flex gap-2 flex-wrap">
        <button onClick={() => setDragEnabled(!dragEnabled)} className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-headline font-semibold border shadow-sm transition-all ${dragEnabled ? 'bg-primary text-white border-primary shadow-md' : 'bg-surface-container-lowest border-surface-container-high text-on-surface-variant hover:text-primary'}`}>{dragEnabled ? <Unlock size={15} /> : <Lock size={15} />}{dragEnabled ? 'Moving' : 'Move'}</button>
        <button onClick={() => setQuickAddOpen(true)} className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-headline font-semibold border shadow-sm bg-surface-container-lowest border-surface-container-high text-on-surface-variant hover:text-primary transition-all"><Plus size={15} /> Add</button>
